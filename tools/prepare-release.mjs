...
const match = /^(gamer-(?:yaml|keymap|video|package-publisher))-v(.+)$/.exec(tag || '')
...
for (const p of registry.plugins) {
  if (!/^gamer-(yaml|keymap|video|package-publisher)$/.test(p.id) || !/^[0-9A-Za-z.+-]+$/.test(p.version)) throw new Error('Invalid artifact name')
